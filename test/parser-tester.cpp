...
TEST_CLASS(ParseTerm)
    TEST_METHOD(ParseIdentExpr)
        ...
        auto identExpr = cast<IdentifierExpr>(cast<ExprStmt>(Parser(ss).getNode())->expr);
    ...
    TEST_METHOD(ParseNumericExpr)
        ...
        Parser parser{ss};
        auto integerExpr = cast<IntegerExpr>(cast<ExprStmt>(parser.getNode())->expr);
        auto floatExpr = cast<FloatExpr>(cast<ExprStmt>(parser.getNode())->expr);
    ...
    TEST_METHOD(ParseNoneExpr)
        ...
        auto noneExpr = cast<NoneExpr>(cast<ExprStmt>(Parser(ss).getNode())->expr);
    TEST_END

    TEST_METHOD(ParseBoolExpr)
        istringstream ss(R"(true false)");
        Parser parser{ss};
        auto trueExpr = cast<BoolExpr>(cast<ExprStmt>(parser.getNode())->expr);
        auto falseExpr = cast<BoolExpr>(cast<ExprStmt>(parser.getNode())->expr);
    ...
    TEST_METHOD(ParseStringExpr)
        ...
        auto strExpr = cast<StringExpr>(cast<ExprStmt>(Parser(ss).getNode())->expr);
        ASSERT(strExpr->value == "Do you love me? Ouch! Of course! \a");
    TEST_END

    TEST_METHOD(ParseEmptyLambdaExpr)
        istringstream ss(R"(@(){})");
        auto lambdaExpr = cast<LambdaExpr>(cast<ExprStmt>(Parser(ss).getNode())->expr);
        ASSERT(lambdaExpr->argDecls.empty());
    ...
    TEST_METHOD(ParseNewExpr)
        ...
        auto newExpr = cast<NewExpr>(cast<ExprStmt>(Parser(ss).getNode())->expr);
    ...
    TEST_METHOD(ParseMatchExpr)
        ...
        Parser parser(ss);
        auto getMatchExpr = [&] {
            return cast<MatchExpr>(cast<ExprStmt>(parser.getNode())->expr); };
        {
    ...
    TEST_METHOD(ParseListExpr)
        ...
        auto listExpr = cast<ListExpr>(cast<ExprStmt>(Parser(ss).getNode())->expr);
    ...
    TEST_METHOD(ParseEnumExpr)
        ...
        Parser parser{ss};
        auto enumExpr = cast<EnumExpr>(cast<ExprStmt>(parser.getNode())->expr);
        ASSERT(enumExpr->idents.size() == 1);
        ASSERT(enumExpr->idents[0]->name == "ONE");
        enumExpr = cast<EnumExpr>(cast<ExprStmt>(parser.getNode())->expr);
    ...
    TEST_METHOD(ParseDictExpr)
        ...
        Parser parser{ss};
        auto dictExpr = cast<DictExpr>(cast<ExprStmt>(parser.getNode())->expr);
        ASSERT(dictExpr->keys.empty() and dictExpr->values.empty());
        dictExpr = cast<DictExpr>(cast<ExprStmt>(parser.getNode())->expr);
